...
@section('title', 'Target Workday')
@section('page_title', 'Target Workday')
@section('page_subtitle', 'Setting target workday bulanan per PT, divisi, dan cabang')
    ...
    <section class="space-y-6">
        ...
        <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            <div class="max-w-3xl">
                ...
                <h2 class="mt-2 text-3xl font-bold tracking-tight text-slate-900">Target Workday Bulanan</h2>
                <p class="mt-3 text-sm leading-7 text-slate-500">
                    Halaman ini dipakai untuk mengatur target workday bulanan dan harian per PT, divisi, serta cabang. Untuk Servanda, pilihan <strong>Semua Divisi</strong> hanya menjadi ringkasan otomatis agar sumber target tetap satu dan dashboard tetap konsisten.
                </p>
            </div>
        </div>

        <form method="GET" action="{{ route('settings.workday-targets.index') }}" class="dashboard-card border border-slate-200 bg-white">
            ...
            <div class="mt-5 flex flex-wrap items-center justify-end gap-3 border-t border-slate-200 pt-4">
                <a
                    href="{{ route('settings.workday-targets.index', ['company' => $selectedCompany]) }}"
        ...
        <form method="POST" action="{{ route('settings.workday-targets.store') }}" class="dashboard-card border border-slate-200 bg-white">
            @csrf
            ...
            @if ($isServandaSummaryMode)
                <div class="mt-5 rounded-3xl border border-sky-200 bg-sky-50/80 px-5 py-4 text-sm leading-7 text-sky-900">
                    ...
                    <p class="mt-1">
                        Nilai bulanan dan harian di bawah ini dihitung dari rata-rata target <strong>Cleaning</strong> dan <strong>Security</strong> yang sudah disimpan pada filter yang sama. Dashboard tetap membaca target per divisi karyawan, jadi ringkasan ini tidak menjadi sumber edit baru.
                    </p>
                </div>
            @endif
                ...
                <table class="min-w-full border-separate border-spacing-y-2">
                    <thead>
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Target Bulanan</th>
                            <th class="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Target Harian</th>
                            @if ($isServandaSummaryMode)
                            ...
                            <tr class="rounded-2xl bg-slate-50/80 shadow-sm">
                                ...
                                <td class="px-4 py-4">
                                    <input
                                        ...
                                        min="0"
                                        max="31"
                                        name="monthly_target[{{ $target['month'] }}]"
                                        value="{{ old('monthly_target.' . $target['month'], $target['monthly_target']) }}"
                                ...
                                <td class="px-4 py-4 {{ $isServandaSummaryMode ? '' : 'rounded-r-2xl' }}">
                                    <input
                                        ...
                                        min="0"
                                        max="31"
                                        name="daily_target[{{ $target['month'] }}]"
                                        value="{{ old('daily_target.' . $target['month'], $target['daily_target']) }}"
                                        @disabled(! $canEditTargets)
                                        @readonly(! $canEditTargets)
                                        class="w-full rounded-2xl border border-slate-200 {{ $canEditTargets ? 'bg-white' : 'bg-slate-100 text-slate-500' }} px-4 py-3 text-sm text-slate-700 shadow-sm"
                                    >
                                        ...
                                        <div class="space-y-2 text-sm text-slate-600">
                                            @foreach ($target['division_breakdown'] as $divisionTarget)
                                                <div class="flex flex-wrap items-center gap-2">
                                                    ...
                                                    <span>Bulanan: {{ $divisionTarget['monthly_target'] !== null ? number_format((float) $divisionTarget['monthly_target'], 2) : '-' }}</span>
                                                    <span>Harian: {{ $divisionTarget['daily_target'] !== null ? number_format((float) $divisionTarget['daily_target'], 2) : '-' }}</span>
                                                </div>
                                            @endforeach
                                        </div>
                                    </td>
