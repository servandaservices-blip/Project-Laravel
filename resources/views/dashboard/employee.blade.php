...
@section('content')
    @php
        ...
        $selectedAreas = collect(request('area', []))->filter()->values()->all();
    ...
    <div
        x-data="{ detailOpen: false, selectedEmployee: null, editOpen: false, editEmployee: null, importOpen: false }"
        @keydown.escape.window="detailOpen = false; editOpen = false; importOpen = false"
        class="relative z-20"
    >
        <section class="dashboard-card overflow-visible">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    ...
                    <p class="mt-2 text-sm leading-6 text-slate-500">
                        Gunakan pencarian dan filter untuk menemukan data karyawan PT {{ $companyName ?? 'Servanda' }} dengan cepat.
                    </p>
                </div>

                <div class="rounded-3xl border border-slate-200 bg-slate-50 px-5 py-4 text-right shadow-sm">
                    <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Total Data</p>
            ...
            <form method="GET" action="{{ route('employee.index') }}" class="relative mt-6 space-y-6">
                ...
                <div class="rounded-3xl border border-slate-200 bg-gradient-to-br from-slate-50 via-white to-blue-50/60 p-5 shadow-sm">
                    <div class="mb-4">
                        <h3 class="text-sm font-semibold uppercase tracking-[0.14em] text-slate-600">Semua Filter</h3>
                        <p class="mt-1 text-sm text-slate-500">Pilih filter yang diperlukan saja agar data lebih fokus dan tidak terlalu ramai.</p>
                    </div>
                    ...
                    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                        <div>
                            ...
                            <details class="group relative z-30" x-data="{ search: '' }">
                                <summary class="flex cursor-pointer list-none items-center justify-between rounded-2xl border border-rose-100 bg-rose-50/70 px-4 py-3 text-sm text-slate-700 transition hover:border-rose-300 group-open:border-rose-400 group-open:ring-4 group-open:ring-rose-100">
                                    <span class="truncate">{{ $multiSelectSummary($selectedBranches, 'Semua') }}</span>
                                    <span class="ml-3 text-rose-500 transition group-open:rotate-180">v</span>
                                </summary>
                        ...
                        @if ($isServanda)
                            <div>
                                ...
                                @else
                                    <select
                                        name="division"
                                        class="w-full rounded-2xl border border-amber-100 bg-amber-50/70 px-4 py-3 text-sm text-slate-700 shadow-sm outline-none transition focus:border-amber-400 focus:ring-4 focus:ring-amber-100"
                                    >
                        ...
                        <div>
                            ...
                            <details class="group relative z-30" x-data="{ search: '' }">
                                <summary class="flex cursor-pointer list-none items-center justify-between rounded-2xl border border-emerald-100 bg-emerald-50/70 px-4 py-3 text-sm text-slate-700 transition hover:border-emerald-300 group-open:border-emerald-400 group-open:ring-4 group-open:ring-emerald-100">
                                    <span class="truncate">{{ $multiSelectSummary($selectedAreas, 'Semua') }}</span>
                                ...
                                <div class="mt-2 rounded-2xl border border-emerald-100 bg-white p-3 shadow-xl shadow-emerald-100/70">
                                    ...
                                    <div class="max-h-56 space-y-2 overflow-y-auto pr-1">
                                        @foreach ($areas as $area)
                                            <label x-show="'{{ $area === $emptyFilterValue ? 'tanpa data' : str($area)->lower()->replace("'", "\\'") }}'.includes(search.toLowerCase())" class="flex items-center gap-3 rounded-xl px-3 py-2 text-sm text-slate-700 transition hover:bg-emerald-50/70">
                                                <input
                                                    type="checkbox"
                                                    name="area[]"
                                                    value="{{ $area }}"
                                                ...
                                                >
                                                <span>{{ $displayFilterValue($area) }}</span>
                                            </label>
                                        @endforeach
                                    </div>
                                </div>
                        ...
                        <div>
                            ...
                            <select
                                name="area_manager"
                                class="w-full rounded-2xl border border-teal-100 bg-teal-50/70 px-4 py-3 text-sm text-slate-700 shadow-sm outline-none transition focus:border-teal-400 focus:ring-4 focus:ring-teal-100"
                            >
                                <option value="">Semua</option>
                        ...
                        <div>
                            ...
                            <select
                                ...
                                class="w-full rounded-2xl border border-cyan-100 bg-cyan-50/70 px-4 py-3 text-sm text-slate-700 shadow-sm outline-none transition focus:border-cyan-400 focus:ring-4 focus:ring-cyan-100"
                            >
                                <option value="">Semua</option>
                        ...
                        <div>
                            ...
                            <details class="group relative z-30" x-data="{ search: '' }">
                                <summary class="flex cursor-pointer list-none items-center justify-between rounded-2xl border border-indigo-100 bg-indigo-50/70 px-4 py-3 text-sm text-slate-700 transition hover:border-indigo-300 group-open:border-indigo-400 group-open:ring-4 group-open:ring-indigo-100">
                                    <span class="truncate">{{ $multiSelectSummary($selectedPayFrequencies, 'Semua') }}</span>
                                    <span class="ml-3 text-indigo-500 transition group-open:rotate-180">v</span>
                                </summary>
                        ...
                        <div>
                            ...
                            <details class="group relative z-30" x-data="{ search: '' }">
                                <summary class="flex cursor-pointer list-none items-center justify-between rounded-2xl border border-cyan-100 bg-cyan-50/70 px-4 py-3 text-sm text-slate-700 transition hover:border-cyan-300 group-open:border-cyan-400 group-open:ring-4 group-open:ring-cyan-100">
                                    <span class="truncate">{{ $multiSelectSummary($selectedContractStatuses, 'Semua') }}</span>
                                    <span class="ml-3 text-cyan-500 transition group-open:rotate-180">v</span>
                                </summary>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('employee.index', ['company' => $selectedCompany ?? 'servanda']) }}" class="rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                        Reset
                    </a>
    ...
    <section class="dashboard-card relative z-10 mt-6 overflow-hidden p-0">
        <div class="border-b border-slate-200 px-6 py-5">
            <h3 class="text-xl font-semibold tracking-tight text-slate-900">Tabel Pegawai {{ $companyName ?? 'Servanda' }}</h3>
            <p class="mt-1 text-sm leading-6 text-slate-500">
                Tabel dibuat lebih ringkas, sementara detail lengkap pegawai bisa dibuka lewat tombol action bergambar mata.
            </p>

            <form method="GET" action="{{ route('employee.index') }}" class="mt-4">
                <input type="hidden" name="company" value="{{ $selectedCompany ?? 'servanda' }}">
                ...
                <div>
                    ...
                    <input
                        ...
                        class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5 text-sm text-slate-700 shadow-sm outline-none transition focus:border-blue-400 focus:bg-white focus:ring-4 focus:ring-blue-100"
                    >
                </div>
            </form>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-[1220px] w-full text-[14px] leading-6">
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    @forelse ($employees as $employee)
                        ...
                        <tr class="transition hover:bg-slate-50/70">
                            ...
                            <td class="min-w-[190px] px-5 py-4 align-top text-slate-700">{{ $employee->position ?: '-' }}</td>
                            ...
                            <td class="min-w-[280px] px-5 py-4 align-top text-slate-700">{{ $employee->area ?: '-' }}</td>
                            <td class="min-w-[190px] px-5 py-4 align-top">
                            ...
                            <td class="min-w-[110px] px-5 py-4 text-center align-top">
        ...
        <section class="dashboard-card relative z-10 mt-6 overflow-hidden">
            <div class="mb-5">
                ...
                <h3 class="text-xl font-semibold tracking-tight text-slate-900">Panduan Status Kontrak</h3>
                <p class="mt-1 text-sm leading-6 text-slate-500">
            ...
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <div class="rounded-3xl border border-emerald-200 bg-gradient-to-br from-emerald-50 via-white to-emerald-100/60 p-5 shadow-sm">
                    <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-200">
                        Aman
                    </span>
                ...
                <div class="rounded-3xl border border-amber-200 bg-gradient-to-br from-amber-50 via-white to-yellow-100/60 p-5 shadow-sm">
                    <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-200">
                        Perhatian
                    </span>
                ...
                <div class="rounded-3xl border border-rose-200 bg-gradient-to-br from-rose-50 via-white to-rose-100/60 p-5 shadow-sm">
                    <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold bg-rose-50 text-rose-700 ring-1 ring-inset ring-rose-200">
                        Menjelang Berakhir
                    </span>
                ...
                <div class="rounded-3xl border border-slate-300 bg-gradient-to-br from-slate-50 via-white to-slate-100/80 p-5 shadow-sm">
                    <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold bg-white text-slate-900 ring-1 ring-inset ring-slate-300">
                        Expired
                    </span>
