...
@section('title', 'Attendance Area')
@section('page_title', 'Attendance Area')
@section('page_subtitle', 'Ringkasan attendance per area')
    ...
    <section x-data="{ areaModalOpen: false, selectedArea: null, areaSearch: '' }" @keydown.escape.window="areaModalOpen = false" class="space-y-6">
        ...
        <section class="dashboard-card border border-slate-200 bg-white">
            <div class="flex flex-col gap-4 border-b border-slate-200 pb-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Area Attendance</p>
                    <h3 class="mt-1 text-xl font-semibold text-slate-900">Tabel Attendance Area</h3>
                    <p class="mt-1 text-sm text-slate-500">Ringkasan attendance per area beserta akses cepat untuk melihat daftar employee di area terkait.</p>
                </div>

                <form method="GET" action="{{ route('dashboard.attendance-area') }}" class="flex flex-wrap items-center gap-2">
                    ...
                    <select name="area_month" onchange="this.form.submit()" class="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-700 shadow-sm">
                        <option value="">Semua Bulan</option>
                        @foreach (range(1, 12) as $monthNumber)
                            <option value="{{ $monthNumber }}" @selected(($selectedAreaMonth ?? null) === $monthNumber)>
                                {{ \Illuminate\Support\Carbon::create()->month($monthNumber)->translatedFormat('F') }}
                            </option>
                        @endforeach
                    </select>
                    <label class="text-sm text-slate-500">Show</label>
                    <select name="area_per_page" onchange="this.form.submit()" class="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-700 shadow-sm">
                        @foreach ([10, 50, 100] as $size)
                            <option value="{{ $size }}" @selected(($selectedAreaPerPage ?? 10) === $size)>{{ $size }}</option>
                        @endforeach
                    </select>
                </form>
            </div>
            ...
            <div class="mt-5 overflow-x-auto">
                <table class="min-w-full border-separate border-spacing-y-2">
                    ...
                    <tbody>
                        @forelse ($areaAttendanceTable as $areaItem)
                            @php
                                $areaPayload = [
                                    'label' => $areaItem['label'],
                                    'employee_count' => $areaItem['employee_count'],
                                    'avg_workday' => number_format($areaItem['avg_workday'], 2),
                                    'avg_present' => number_format($areaItem['avg_present'], 2),
                                    'avg_absent' => number_format($areaItem['avg_absent'] ?? 0, 2),
                                    'attendance_rate' => number_format($areaItem['attendance_rate'], 2),
                                    'employees' => $areaItem['employees']->map(function ($employee) {
                                        return [
                            ...
                            @endphp
                            <tr x-show="@js(strtolower($areaItem['label'])).includes(areaSearch.toLowerCase())" x-transition.opacity class="rounded-2xl bg-slate-50/80 shadow-sm">
                                <td class="rounded-l-2xl px-4 py-4 align-top">
                                    <p class="font-semibold text-slate-900">{{ $areaItem['label'] }}</p>
                                    <p class="mt-1 text-xs text-slate-500">{{ $areaItem['employee_count'] }} employee</p>
                                </td>
                                <td class="px-4 py-4 text-center text-sm font-medium text-slate-800">{{ number_format($areaItem['avg_workday'], 2) }}</td>
                                <td class="px-4 py-4 text-center text-sm font-medium text-sky-700">{{ number_format($areaItem['avg_present'], 2) }}</td>
                                <td class="px-4 py-4 text-center text-sm font-medium text-slate-800">{{ number_format($areaItem['avg_absent'] ?? 0, 2) }}</td>
                                <td class="px-4 py-4 text-center text-sm font-semibold {{ $areaItem['attendance_rate'] >= 90 ? 'text-emerald-700' : ($areaItem['attendance_rate'] >= 80 ? 'text-amber-700' : 'text-rose-700') }}">
                                    {{ number_format($areaItem['attendance_rate'], 2) }}%
                                </td>
                                <td class="rounded-r-2xl px-4 py-4 text-center">
                                    <button
                                        type="button"
                                        x-on:click.prevent="selectedArea = @js($areaPayload); areaModalOpen = true"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="rounded-2xl border border-dashed border-slate-200 px-4 py-10 text-center text-sm text-slate-500">
                                    Belum ada data attendance area.
                                </td>
                            </tr>
                        @endforelse
                        @if ($areaAttendanceTable->count() > 0)
                            <tr
            ...
            <div class="mt-6 flex flex-col gap-3 border-t border-slate-200 pt-4 md:flex-row md:items-center md:justify-between">
                <p class="text-sm text-slate-500">
                    Menampilkan {{ $areaAttendanceTable->firstItem() ?? 0 }} - {{ $areaAttendanceTable->lastItem() ?? 0 }} dari {{ $areaAttendanceTable->total() }} area.
                    @if (! empty($selectedAreaMonth))
                        Data bulan {{ \Illuminate\Support\Carbon::create()->month($selectedAreaMonth)->translatedFormat('F') }}.
                    @else
                        Data rata-rata tahun {{ $selectedYear }}.
                    @endif
                </p>
                <div>
                    {{ $areaAttendanceTable->onEachSide(1)->links() }}
                </div>
            </div>
        </section>
        ...
        >
            ...
            >
                <div class="flex items-start justify-between gap-4 border-b border-slate-200 bg-[radial-gradient(circle_at_top_left,_rgba(14,165,233,0.20),_transparent_32%),linear-gradient(135deg,#eff6ff_0%,#ffffff_55%,#ecfeff_100%)] px-6 py-5">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-sky-700">Detail Area</p>
                        <h3 class="mt-2 text-xl font-bold tracking-tight text-slate-900" x-text="selectedArea?.label || '-'"></h3>
                        <p class="mt-2 text-sm text-slate-600">
                            Detail employee yang masuk dalam area ini berdasarkan data attendance tahunan.
                        </p>
                    </div>

                    <button
                        type="button"
                        @click="areaModalOpen = false"
                ...
                <div class="border-b border-slate-200 bg-slate-50 px-6 py-4">
                    <div class="grid gap-3 md:grid-cols-5">
                        <div class="rounded-2xl bg-white px-4 py-3 shadow-sm">
                            ...
                            <p class="mt-1 font-bold text-slate-900" x-text="selectedArea?.employee_count || '0'"></p>
                        </div>
                        <div class="rounded-2xl bg-white px-4 py-3 shadow-sm">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">Avg Workday</p>
                            <p class="mt-1 font-bold text-slate-900" x-text="selectedArea?.avg_workday || '0.00'"></p>
                        </div>
                        <div class="rounded-2xl bg-white px-4 py-3 shadow-sm">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">AVG Jadwal Shift</p>
                            <p class="mt-1 font-bold text-slate-900" x-text="selectedArea?.avg_present || '0.00'"></p>
                        </div>
                        <div class="rounded-2xl bg-white px-4 py-3 shadow-sm">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">Tidak Absen</p>
                            <p class="mt-1 font-bold text-slate-900" x-text="selectedArea?.avg_absent || '0.00'"></p>
                        </div>
                        <div class="rounded-2xl bg-white px-4 py-3 shadow-sm">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">Attendance</p>
                            <p class="mt-1 font-bold text-slate-900" x-text="(selectedArea?.attendance_rate || '0.00') + '%'"></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="max-h-[65vh] overflow-y-auto px-6 py-6">
                    <div class="overflow-x-auto">
                        <table class="min-w-full border-separate border-spacing-y-2">
                            ...
                            <tbody>
                                <template x-if="!selectedArea || !selectedArea.employees || selectedArea.employees.length === 0">
                                    <tr>
                                        <td colspan="7" class="rounded-2xl border border-dashed border-slate-200 px-4 py-10 text-center text-sm text-slate-500">
                                            Belum ada employee pada area ini.
                                        </td>
                                    </tr>
                                </template>
                                <template x-for="employee in (selectedArea?.employees || [])" :key="employee.employee_no + employee.employee_name">
